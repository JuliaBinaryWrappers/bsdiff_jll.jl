...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import bsdiff_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("63b17cf3-85a2-53ea-9ee0-a1ce3782c3ad"))
platforms = [Pkg.Artifacts.unpack_platform(e, "bsdiff", artifacts_toml) for e in artifacts["bsdiff"]]
...
if best_platform === nothing
    @debug("Unable to load bsdiff; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module bsdiff_jll
